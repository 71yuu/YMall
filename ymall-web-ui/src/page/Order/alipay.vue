<template>
  <div class="w" style="padding-bottom: 100px;">
    <y-shelf title="XPay收银台 收款方: yuu">
      <div slot="content">
        <div class="box-inner order-info">
          ...
          <div class="qrguide-area" v-if="!custom">
    ...
    <el-dialog
      ...
      @close="handleClose" style="text-align:center;margin:0 auto">
    ...
    <el-dialog
      ...
      @close="handleCloseCus" style="text-align:center;margin:0 auto">
      支付时请在备注中输入您的订单标识号：<span class="payNum">{{payNum}}</span><br>
      若忘记输入或输入错误可能会造成您支付失败！
      <img src="static/images/aliremark.png" width="350px" class="display: block;margin: 0 auto;"/>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="customVisible = false">知道了</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    mounted () {
      ...
      if (isCustom !== 'true') {
        ...
        this.imgPath = 'static/qr/alipay/' + this.picName + '/' + payNum + '.png'
      } else {
        this.custom = true
        this.payNum = payNum
      }
